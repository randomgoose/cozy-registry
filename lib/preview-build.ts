...
function collectTailwindClassCandidatesFromSource(source: string): string[] {
  ...
  const patterns = [
    ...
    /\b(?:cn|clsx|twMerge|classNames|cva)\(([\s\S]*?)\)/g,
  ];
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(source)) !== null) {
      const raw = match[1] ?? "";
      if (!raw) continue;
      if (pattern.source.includes("cn|clsx|twMerge|classNames|cva")) {
...
async function buildTailwindBaselineCss(
  ...
  const compiled = await tailwind.compile(
    [
      '@import "tailwindcss";',
      "@theme inline {",
      ...
      "  --color-input-background: var(--input-background, var(--card));",
      "  --color-ring: var(--ring);",
