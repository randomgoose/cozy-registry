...
import { z } from "zod";
import { verifySignedObject } from "./crypto";
...
/** Vercel request logs only show HTTP status — JSON-RPC errors often still use 200 OK. */
...
export async function handleMcpRequest(request: Request, origin: string): Promise<Response> {
  ...
  // Match main registry: do not require absence of Mcp-Session-Id — some clients send it on the
  // post-OAuth probe; skipping the short-circuit then hits the SDK (e.g. 406 Accept) and the UI spins.
