...
```json
{
  ...
  "items": {
    "@acme/hero-section": {
      ...
      "installedFiles": [
        "src/components/registry/acme/hero-section/index.tsx",
        "src/components/registry/acme/hero-section/webgl.ts",
        "src/components/registry/acme/hero-section/styles.css"
      ],
...
## 依赖安装与 Provenance（Design System 组件依赖）

本节描述当一个组件依赖另一个 registry 组件时，项目侧安装协议应如何记录与落盘，以支持：

- 组件间依赖（`registryDependencies`）的可追踪与可升级
- “本地展开依赖用于运行/预览，但发布时不产生两份依赖实现”的协作流（stub + `_deps`）

### 1) lockfile 记录（推荐）

当安装 `@owner/name` 后，lockfile 应记录其依赖（若 registry 提供）：

- `items[@owner/name].registryDependencies`: string[]
- `items[@owner/name].themeDependencies`: string[]（可选，便于快速注入/诊断）

说明：

- `registryDependencies` 是 registry ref（`@owner/dep[@version]`），不是 npm `dependencies`。
- 未写入时可通过 resolver 再算，但写入能提升调试与离线可用性。

### 2) Provenance 文件（已实现的安装产物）

每个安装目录应写入一个 provenance manifest（示例名：`cozy.provenance.json`），用于后续发布时 de-vendor：

- 标记哪些文件来自 registry 展开（`source:"registry"`）
- 记录对应依赖 ref 与 content hash（用于 strict 模式 dirty 检测）

建议位置（与安装目录同级）：

`src/components/registry/{owner}/{name}/cozy.provenance.json`

### 3) 依赖安装（V1）

V1 采用**平铺安装**，不在根组件目录下额外创建 `_deps/`。

当 `@owner/dialog` 依赖 `@owner/button` 时，安装结果应分别落到两个独立 item 目录：

```text
src/components/registry/owner/dialog/index.tsx
src/components/registry/owner/button/index.tsx
```

说明：

- 依赖关系通过 `cozy-registry.lock.json` 中的 `registryDependencies` 表达，而不是通过目录嵌套表达。
- 安装器需要在必要时重写跨 item import，使根组件源码指向已安装的依赖目录。
- 主题（`registry:theme`）在 V1 中也按普通 item 平铺安装，不做 `_themes/` 特殊目录。

### 4) 升级语义（建议）

- 根组件升级时：
  - 若依赖 ref 浮动（未 pin），升级可能隐式改变依赖版本；建议在生产/稳定环境 pin 版本。
- 依赖组件升级时：
  - 作为独立 item 升级；根组件的 lockfile 关系保持可追踪。

### 5) 失败策略（建议）

- 依赖缺失/不可访问：
  - 安装/升级应显式失败，并给出缺失 ref 与修复建议。
  - 预览可提供诊断模式，但不应静默吞错导致行为不确定。
