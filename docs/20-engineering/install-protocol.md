...
`src/registry/{owner}/{name}/cozy.provenance.json`

### 3) Stub + `_deps`（安装侧建议）

当组件源码通过相对路径引用依赖文件（如 `import { Button } from "./Button"`），项目侧安装可采用：

- 在根组件目录下保留 `Button.tsx` 作为 **stub**
- 将真实依赖源码物化到 `_deps/<owner>/<dep>/...`

示例结构：

```text
src/registry/chen/dialog/index.tsx
src/registry/chen/dialog/Button.tsx              # stub
src/registry/chen/dialog/_deps/chen/button/...   # materialized dependency sources
```
...
export * from "./_deps/chen/button/index";
```

注意：

- `_deps` 必须是稳定相对路径，避免 `../../` 复杂计算。
- 依赖物化目录建议包含版本以避免升级冲突（例如 `_deps/chen/button@1.2.0/`）。
...
- 依赖组件升级时：
  - 物化目录按版本隔离，避免覆盖造成“旧 stub 指向新实现”的不可预期行为。
