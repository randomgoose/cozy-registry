...
When a stub is detected, the system SHOULD:

- infer the corresponding registry ref and add it to `registryDependencies` (deduped), or
- at minimum emit a warning/diagnostic that the dependency graph may be incomplete.

Notes:

- Stub scanning must be best-effort and MUST NOT be the only mechanism (it is less reliable than provenance).
- Stub scanning must not override explicit `registryDependencies` declarations; it should only merge.
