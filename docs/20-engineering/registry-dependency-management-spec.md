...
1. **Only include** `source: "root"` files in the uploaded `files` bundle.
2. **Derive** `registryDependencies` from `source: "registry"` entries by collecting their `ref` (deduped).
3. Do **not** publish `source: "generated"` files.
