...
import { signInWithEmail, startSocialSignIn } from "../../lib/auth-control";
import { FigmaAuthIcon } from "../icons";
...
export function SignInPage() {
  ...
  const callbackURL = useMemo(
    () => new URLSearchParams(search).get("callbackUrl") ?? DEFAULT_CALLBACK,
    [search],
  );
  const signUpHref =
  ...
  async function handleSocial(provider: "google" | "figma") {
    setError("");
    setLoading(true);
    try {
      await startSocialSignIn({ provider, callbackURL });
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : "Social sign-in failed");
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { response, data } = await signInWithEmail({
        email,
        password,
        callbackURL,
      });
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 px-6 dark:bg-zinc-950">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          Sign in to publish and manage your registry assets.
        </p>
        ...
        <div className="space-y-2">
          ...
          <button
            type="button"
            onClick={() => void handleSocial("figma")}
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-[#0d99ff] bg-[#0d99ff] px-4 py-2 text-sm font-medium text-white transition-colors hover:border-[#0b8ae5] hover:bg-[#0b8ae5]"
          >
        ...
        <p className="mt-4 text-center text-sm text-zinc-600 dark:text-zinc-400">
          ...
          <a href={signUpHref} className="font-medium text-blue-600 hover:underline dark:text-blue-400">
            Sign up
          </a>
        </p>
      </div>
