...
function buildHtmlResponse(html: string, status = 200): Response {
...
function buildDependencyErrorResponse(input: {
...
<html lang="en" style="${input.previewMode === "thumbnail" ? "background:transparent;" : ""}">
  <head>
    <meta charset="UTF-8" />
  ...
  <body style="margin:0;padding:24px;font-family:system-ui,-apple-system,BlinkMacSystemFont,&quot;Segoe UI&quot;,sans-serif;background:#fef2f2;color:#b91c1c;">
    ...
    <p style="font-size:13px;margin:0 0 8px;"><strong>${escapeHtml(input.code)}</strong></p>
    <pre style="white-space:pre-wrap;font-size:13px;background:#fff;border-radius:8px;border:1px solid #fecaca;padding:12px;color:#991b1b;">${escapeHtml(input.message)}${input.cyclePath ? "\n\n" + escapeHtml(input.cyclePath.join(" -> ")) : ""}</pre>
  </body>
</html>`;
  return buildHtmlResponse(html, 500);
}

function buildBuildErrorResponse(input: {
  previewMode: PreviewMode;
  message: string;
  details: string;
}) {
...
<html lang="en" style="${input.previewMode === "thumbnail" ? "background:transparent;" : ""}">
  ...
  <body style="margin:0;padding:24px;font-family:system-ui,-apple-system,BlinkMacSystemFont,&quot;Segoe UI&quot;,sans-serif;background:#fef2f2;color:#b91c1c;">
    ...
    <pre style="white-space:pre-wrap;font-size:13px;background:#fff;border-radius:8px;border:1px solid #fecaca;padding:12px;color:#991b1b;">${input.message}${input.details ? "\\n" + input.details : ""}</pre>
  </body>
</html>`;
  return buildHtmlResponse(html, 500);
}

export async function resolveLegacyPreviewRedirect(input: {
  context: Pick<PlatformRequestContext, "userId">;
  nameFromPath: string;
}) {
  const item = await getRegistryItemByName(input.nameFromPath, input.context.userId);
  if (!item) {
    return null;
  }
...
export async function renderPreviewResponse(input: {
  request: Request;
  owner: string;
  name: string;
  ...
  const context = input.context ?? (await getPlatformRequestContext(input.request));
  ...
  try {
    const rows = await getRegistryItemVersions(input.owner, input.name, userId);
...
<html lang="en" style="${previewMode === "thumbnail" ? "background:transparent;" : ""}">
  <head>
    <meta charset="UTF-8" />
    <title>Theme: ${item.title ?? input.name}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
...
${versionToolbarHtml}
  ...
  if (rawPreviewProps === undefined || rawPreviewProps === null) {
    previewProps = DEMO_PROPS[input.name] ?? {};
  ...
  try {
    ...
    const materialized = materializeComponentSourceFilesFromResolvedGraph(
      ...
      { owner: input.owner, name: input.name, version },
    );
    componentDepSources = materialized.sources;
    for (const [path, content] of Object.entries(materialized.files)) {
    ...
    return buildDependencyErrorResponse({
      previewMode,
      code,
      message: err instanceof Error ? err.message : String(err),
      cyclePath: err instanceof RegistryDependencyCycleError ? err.path : undefined,
    });
  }
    ...
    if (!buildResult.ok) {
      ...
      const details =
        err.file && err.line != null
          ...
          : "";
      return buildBuildErrorResponse({
        previewMode,
        message: err.message,
        details,
      });
  ...
  for (const dep of runtimeDependencies) {
    if (!dep || dep in importMap) continue;
